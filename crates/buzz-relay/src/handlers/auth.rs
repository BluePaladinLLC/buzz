...
//! handler independently verifies any presented credential so direct members of
//! closed relays can also materialize their proven owner relationship.
//!
...
pub async fn handle_auth(event: nostr::Event, conn: Arc<ConnectionState>, state: Arc<AppState>) {
        ...
        .await
    {
        Ok(mut auth_ctx) => {
            ...
            // Independently extract a cryptographically valid NIP-OA owner after
            // membership admission. A direct member of a closed relay otherwise
            // short-circuits membership delegation and never materializes its
            // owner mapping even though the signed AUTH event carries the proof.
